...
from .backends.base import is_raspberry_pi_5, get_pi_model
...
class PiWave:
    def __init__(self, 
        ...
        :type loop: bool
        :param backend: Chose a specific backend to handle the broadcast (default: auto). Supports `pi_fm_rds`, `fm_transmitter`, `pi5_fm` (Pi 5 experimental, RP1) and `auto`
        :type backend: str
        :param used_for: Give the main use for the current instance, will be used if backend: auto (default: file_broadcast). Supports `file_broadcast` and `live_broadcast`
        :type used_for: str
        ...
        if backend == "auto":
            ...
            if not backend_name:
                ...
                if is_raspberry_pi_5():
                    raise PiWaveError(
                        f"No suitable backend found for {self.frequency}MHz on Raspberry Pi 5. "
                        f"Available backends: {available}. "
                        f"pi_fm_rds/fm_transmitter are BCM-only and cannot drive Pi 5 header pins "
                        f"(GPIO moved to RP1). Install the experimental 'pi5_fm' backend "
                        f"(c_src/pi5_fm/README.md), antenna on GPIO20/21, not GPIO4.")
            ...
            # BCM backends cannot work on Pi 5 even if a stale cache/binary exists.
            if is_raspberry_pi_5() and backend in ("pi_fm_rds", "fm_transmitter"):
                raise PiWaveError(
                    f"Backend '{backend}' does not support Raspberry Pi 5 (BCM GPCLK/DMA to GPIO4 "
                    f"has no path to RP1 header pins). Use backend='pi5_fm' with antenna on "
                    f"GPIO20 (GPCLK0) / GPIO21 (GPCLK1). See c_src/pi5_fm/README.md.")
    ...
    def _is_raspberry_pi(self) -> bool:
        # Pi 5 /proc/cpuinfo no longer contains "Raspberry", so check
        # /proc/device-tree/model first (reliable on Pi 4/5).
        try:
    ...
    def _is_raspberry_pi_5(self) -> bool:
        return is_raspberry_pi_5()

    def _get_pi_model(self) -> str:
        return get_pi_model() or "unknown"
