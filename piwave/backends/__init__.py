...
from .pi5_fm import Pi5FmBackend
from .base import is_raspberry_pi_5
...
def get_best_backend(mode: str, frequency: float):
    # On Pi 5 only pi5_fm is valid (BCM backends can't reach header pins).
    if is_raspberry_pi_5():
        if "pi5_fm" in backends:
            backend = backends["pi5_fm"]()
...
def discover_backends():
    ...
    for name, backend_class in backend_classes.items():
        # Never trust a cached BCM backend on Pi 5 (SD card moved from older Pi).
        if is_raspberry_pi_5() and not backend_class.supports_pi5:
...
def search_backends():
    ...
    for name, backend_class in backend_classes.items():
        ...
        try:
            available = backend.is_available(force=True)
        except Exception:
            available = False
    ...
    if backends:
        ...
        if is_raspberry_pi_5():
            Log.warning("No backends found. On Pi 5 only 'pi5_fm' works "
                        "(pi_fm_rds/fm_transmitter are BCM-only). See c_src/pi5_fm/README.md.")
        else:
