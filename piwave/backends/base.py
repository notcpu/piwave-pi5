...
def get_pi_model() -> str:
    # /proc/device-tree/model is the reliable source on Pi 4/5 (Pi 5 cpuinfo
    # no longer contains "Raspberry"). Returns "" when unavailable (e.g. Windows).
    try:
...
def is_raspberry_pi_5() -> bool:
    return 'raspberry pi 5' in get_pi_model().lower()
...
class Backend(ABC):
    # Set to True only by backends driving RP1 (Pi 5) hardware.
    # BCM283x/2711 bit-banging backends (pi_fm_rds, fm_transmitter) stay False.
    supports_pi5: bool = False
    ...
    def _find_executable(self, force: bool = False) -> str:
        # Check cache first
        ...
        if self.cache_file.exists():
            try:
                if force:
                    Log.debug(f"Force search enabled, unlinking old cache.")
    ...
    def is_available(self, force: bool = False):
        if not force and self.is_available_cached is not None:
            return self.is_available_cached

        # BCM GPCLK/DMA bit-banging cannot reach header pins on Pi 5 (GPIO
        # moved to RP1 over PCIe). Fail fast instead of transmitting garbage.
        if not self.supports_pi5 and is_raspberry_pi_5():
            Log.warning(f"Backend '{self.name}' does not support Raspberry Pi 5 "
                        f"(BCM-only, needs RP1). Use 'pi5_fm' instead.")
            self.is_available_cached = False
            return False

        try:
            # Pass force flag down to the executable searcher
            if force:
                self._executable_path = self._find_executable(force=True)
            else:
